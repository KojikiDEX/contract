...
/*
 * This contract is used to distribute dividends to users that allocated xSake here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xSake allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IxSakeTokenUsage, IDividends {
  ...
  address public immutable xSakeToken; // xSakeToken contract

  mapping(address => uint256) public usersAllocation; // User's xSake allocation
  ...
  uint256 internal _cycleDurationSeconds = 72 hours;
  uint256 public currentCycleStartTime;

  constructor(address xSakeToken_, uint256 startTime_) {
    require(xSakeToken_ != address(0), "zero address");
  ...
  modifier xSakeTokenOnly() {
    require(msg.sender == xSakeToken, "xSakeTokenOnly: caller should be xSakeToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of xSake to this dividends contract
   *
  ...
  /**
   * Deallocates "userAddress" user's "amount" of xSake allocation from this dividends contract
   *
   * Can only be called by xSakeToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IxSakeTokenUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no xSake is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userxSakeAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userxSakeAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userxSakeAllocation.mul(accDividendsPerShare).div(1e18);
