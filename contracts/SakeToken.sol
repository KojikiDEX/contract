...
/*
 * Sake is Kojiki's native ERC20 token.
 * It has an hard cap and manages its own emissions and allocations.
 */
contract SakeToken is Ownable, ERC20("Sake Token", "SAKE"), ISakeToken {
  using SafeMath for uint256;

  uint256 public constant MAX_EMISSION_RATE = 0.01 ether;
  uint256 public constant MAX_SUPPLY_LIMIT = 1400000 ether;
   ...
   */
  modifier onlyMaster() {
    require(msg.sender == masterAddress, "SakeToken: caller is not the master");
    _;
  }
  ...
  /**
   * @dev Burns "amount" of Sake by sending it to BURN_ADDRESS
   */
  ...
  /**
   * @dev Updates Sake emission rate per second
   *
   * Must only be called by the owner
   */
