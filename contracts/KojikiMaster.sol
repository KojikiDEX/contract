...
/*
 * This contract centralizes Kojiki's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the SakeToken contract.
 */
contract KojikiMaster is Ownable, IKojikiMaster {
  ...
  ISakeToken private immutable _sakeToken; // Address of the Sake token contract
  ...
  constructor(
    ISakeToken sakeToken_,
    uint256 startTime_
  ) {
    require(address(sakeToken_) != address(0), "KojikiMaster: SakeToken cannot be set to zero address");
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= sakeToken_.lastEmissionTime(), "KojikiMaster: invalid startTime");
    _sakeToken = sakeToken_;
    startTime = startTime_; // Must be set with the same time as SakeToken emission start
  }
  ...
  /*
   * @dev Returns SakeToken address
   */
  function sakeToken() external view override returns (address) {
    return address(_sakeToken);
  }

  /*
   * @dev Returns SakeToken's emission rate (allocated to this contract)
   */
  function emissionRate() public view returns (uint256) {
    return _sakeToken.masterEmissionRate();
  }

  /**
   * @dev Returns current owner's address
   */
  function owner() public view virtual override(IKojikiMaster, Ownable) returns (address) {
  ...
  /**
   * @dev Updates time at which farming starts 
   * Must only be called by the owner
   */
  function setStartTime(uint256 startTime_) external onlyOwner {
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= _sakeToken.lastEmissionTime(), "KojikiMaster: invalid startTime");
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256 effectiveAmount) {
    uint256 sakeBalance = _sakeToken.balanceOf(address(this));

    if (amount > sakeBalance) {
      amount = sakeBalance;
    }
  ...
  function _updatePool(address poolAddress) internal {
    ...
    if (allocPoint > 0 && INFTPool(poolAddress).hasDeposits()) {
      // calculate how much Sake rewards are expected to be received for this pool
      ...
      (rewards) = _sakeToken.claimMasterRewards(rewards);
