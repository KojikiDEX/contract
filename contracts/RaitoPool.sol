...
import "./interfaces/IRaitoPoolFactory.sol";
import "./interfaces/tokens/ISakeToken.sol";
import "./interfaces/tokens/ISakeStakeToken.sol";
import "./interfaces/IRaitoCustomReq.sol";


contract RaitoPool is ReentrancyGuard, Ownable, INFTHandler {
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;
    using SafeERC20 for ISakeStakeToken;
    ...
    struct Settings {
        ...
        string description; // Project's description for this RaitoPool
    }
    ...
    IRaitoPoolFactory public factory; // RaitoPoolFactory address
    ISakeToken public sakeToken; // SakeToken contract
    ISakeStakeToken public sakeStakeToken; // sakeStakeToken contract
    INFTPool public nftPool; // NFTPool contract
    IRaitoCustomReq public customReqContract; // (optional) external contracts allow to handle custom requirements

    uint256 public creationTime; // Creation time of this RaitoPool

    bool public published; // Is RaitoPool published
    uint256 public publishTime; // Time at which the RaitoPool was published
    ...
    constructor(
        ISakeToken _sakeToken, ISakeStakeToken sakeStakeToken_, address owner_, INFTPool nftPool_,
        IERC20 rewardsToken1_, IERC20 rewardsToken2_, Settings memory settings_
    ) {
        require(address(_sakeToken) != address(0) && address(sakeStakeToken_) != address(0) && owner_ != address(0)
        ...
        factory = IRaitoPoolFactory(msg.sender);
    ...
    function setEnableWithdraw(bool enable_) external onlyOwner {
        withrawEnabled = enable_;
    }

    /**
     * @dev Withdraw a position from the RaitoPool
     *
    ...
    function withdraw(uint256 tokenId) external virtual nonReentrant {
        ...
        if (!withrawEnabled) {
            require(_currentBlockTimestamp() > settings.endTime, "pool not ended");
        }
    ...
    /**
     * @dev Withdraw a position from the RaitoPool without caring about rewards, EMERGENCY ONLY
     *
    ...
    /**
     * @dev Harvest pending RaitoPool rewards
     */
     ...
     * "to" can be set to this address only if this contract is allowed to transfer sakeStakeToken
     */
    function onNFTHarvest(address operator, address to, uint256 tokenId, uint256 sakeTokenAmount, uint256 sakeStakeTokenAmount) external override isValidNFTPool(msg.sender) returns (bool) {
        address owner = tokenIdOwner[tokenId];
        require(operator == owner, "not allowed");

        // if not whitelisted, the RaitoPool can't transfer any sakeStakeToken rewards
        require(to != address(this) || sakeStakeToken.isTransferWhitelisted(address(this)), "cant handle rewards");

        // redirect rewards to position's previous owner
        if (to == address(this)) {
            sakeToken.safeTransfer(owner, sakeTokenAmount);
            sakeStakeToken.safeTransfer(owner, sakeStakeTokenAmount);
        }
    ...
    /**
     * @dev Transfer ownership of this RaitoPool
     *
    ...
    /**
     * @dev Add rewards to this RaitoPool
     */
    function addRewards(uint256 amountToken1, uint256 amountToken2) external nonReentrant {
        ...
        // get active fee share for this RaitoPool
    ...
    /**
     * @dev Withdraw rewards from this RaitoPool
     *
    ...
    function setCustomReqContract(address contractAddress) external onlyOwner {
        ...
        customReqContract = IRaitoCustomReq(contractAddress);
