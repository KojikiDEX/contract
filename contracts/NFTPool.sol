...
/*
 * This contract wraps ERC20 assets into non-fungible staking positions called ksNFTs
 * ksNFTs add the possibility to create an additional layer on liquidity providing lock features
 * ksNFTs are yield-generating positions when the NFTPool contract has allocations from the Kojiki Master
 */
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("KamiStakedNFT", "ksNFT"), ERC721Enumerable {
  ...
  struct StakingPosition {
    ...
    uint256 boostPoints; // Allocated sakeStakeToken from yieldboost contract (optional)
    uint256 totalMultiplier; // lockMultiplier + allocated sakeStakeToken boostPoints multiplier
    uint256 pendingSakeStakeTokenRewards; // Not harvested sakeStakeToken rewards
    uint256 pendingSakeTokenRewards; // Not harvested Sake Token rewards
  }

  Counters.Counter private _tokenIds;

  EnumerableSet.AddressSet private _unlockOperators; // Addresses allowed to forcibly unlock locked ksNFTs
  address public operator; // Used to delegate multiplier settings to project's owners
  IKojikiMaster public master; // Address of the master
  ...
  IERC20 private _sakeToken; // SakeToken contract's address
  ISakeStakeToken private _sakeStakeToken; // SakeStakeToken contract's address
  ...
  uint256 private _maxBoostMultiplier = 10000; // 100%, Max boost that can be earned from sakeStakeToken yieldBooster

  uint256 private constant _TOTAL_REWARDS_SHARES = 10000; // 100%, high limit for sakeStakeTokenRewardsShare
  uint256 public sakeStakeTokenRewardsShare = 8000; // 80%, directly defines sakeTokenShare with the remaining value to 100%
  ...
  function initialize(IKojikiMaster master_, IERC20 sakeToken, ISakeStakeToken sakeStakeToken, IERC20 lpToken) external {
    require(msg.sender == factory && !initialized, "FORBIDDEN");
    _lpToken = lpToken;
    master = master_;
    _sakeToken = sakeToken;
    _sakeStakeToken = sakeStakeToken;
    initialized = true;

    // to convert Sake to sakeStakeToken
   _sakeToken.approve(address(_sakeStakeToken), type(uint256).max);
  }
  ...
  event SetSakeStakeTokenRewardsShare(uint256 sakeStakeTokenRewardsShare);
  ...
  /**
   * @dev Check if a userAddress has privileged rights on a ksNFT
   */
  ...
  /**
   * @dev Check if a msg.sender is owner of a ksNFT
   */
  ...
  /**
   * @dev Returns true if "tokenId" is an existing ksNFT id
   */
   ...
   */
  function lastTokenId() external override view returns (uint256) {
  ...
  function getPoolInfo() external view override returns (
    address lpToken, address sakeToken, address sakeStakeToken, uint256 lastRewardTime, uint256 accRewardsPerShare,
    ...
    return (
    address(_lpToken), address(_sakeToken), address(_sakeStakeToken), lastRewardTime, _accRewardsPerShare,
  ...
  function pendingRewards(uint256 tokenId) external view returns (uint256) {
    ...
    return position.amountWithMultiplier.mul(accRewardsPerShare).div(1e18).sub(position.rewardDebt)
      .add(position.pendingSakeStakeTokenRewards).add(position.pendingSakeTokenRewards);
  }
  ...
  /**
   * @dev Set the share of sakeStakeToken for the distributed rewards
   * The share of Sake will incidently be 100% - sakeStakeTokenRewardsShare
   *
   * Must only be called by the owner
   */
  function setSakeStakeTokenRewardsShare(uint256 sakeStakeTokenRewardsShare_) external {
    _requireOnlyOwner();
    require(sakeStakeTokenRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

    sakeStakeTokenRewardsShare = sakeStakeTokenRewardsShare_;
    emit SetSakeStakeTokenRewardsShare(sakeStakeTokenRewardsShare_);
  }
  ...
  /**
   * @dev Create a staking position (ksNFT) with an optional lockDuration
   */
  function createPosition(uint256 amount, uint256 lockDuration) external override nonReentrant {
    ...
    _stakingPositions[currentTokenId] = StakingPosition({
      ...
      pendingSakeTokenRewards: 0,
      pendingSakeStakeTokenRewards: 0
    });
   ...
   *
   * Can only be called by ksNFT's owner or operators
   */
   ...
   *
   * Can only be called by ksNFT's owner or approved address
   */
   ...
   *
   * Can only be called by ksNFT's owner or approved address
   * ksNFT's owner must be a contract
   */
   ...
   *
   * Can only be called by ksNFT's owner
   */
  ...
  /**
   * @dev Returns whether "userAddress" is the owner of "tokenId" ksNFT
   */
   ...
   * Users should still be able to deallocate sakeStakeToken from the YieldBooster contract
   */
  function _destroyPosition(uint256 tokenId, uint256 boostPoints) internal {
    // calls yieldBooster contract to deallocate the ksNFT's owner boost points if any
    if (boostPoints > 0) {
  ...
  /**
   * @dev Computes new tokenId and mint associated ksNFT to "to" address
   */
  ...
  function _harvestPosition(uint256 tokenId, address to) internal {
    ...
    // transfer rewards
    if (pending > 0 || position.pendingSakeStakeTokenRewards > 0 || position.pendingSakeTokenRewards > 0) {
      uint256 sakeStakeTokenRewards = pending.mul(sakeStakeTokenRewardsShare).div(_TOTAL_REWARDS_SHARES);
      uint256 sakeTokenAmount = pending.add(position.pendingSakeTokenRewards).sub(sakeStakeTokenRewards);

      sakeStakeTokenRewards = sakeStakeTokenRewards.add(position.pendingSakeStakeTokenRewards);

      // Stack rewards in a buffer if to is equal to address(0)
      if (address(0) == to) {
        position.pendingSakeStakeTokenRewards = sakeStakeTokenRewards;
        position.pendingSakeTokenRewards = sakeTokenAmount;
      }
      else {
        // convert and send sakeStakeToken + Sake rewards
        position.pendingSakeStakeTokenRewards = 0;
        position.pendingSakeTokenRewards = 0;

        if(sakeStakeTokenRewards > 0) sakeStakeTokenRewards = _safeConvertTo(to, sakeStakeTokenRewards);
        // send share of Sake rewards
        sakeTokenAmount= _safeRewardsTransfer(to, sakeTokenAmount);

        // forbidden to harvest if contract has not explicitly confirmed it handle it
        _checkOnNFTHarvest(to, tokenId, sakeTokenAmount, sakeStakeTokenRewards);
      }
    }
  ...
  */
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256) {
    uint256 balance = _sakeToken.balanceOf(address(this));
    // cap to available balance
    if (amount > balance) {
      amount = balance;
    }
    _sakeToken.safeTransfer(to, amount);
    return amount;
  }

  /**
   * @dev Safe convert Sake to sakeStakeToken function, in case rounding error causes pool to not have enough tokens
   */
  function _safeConvertTo(address to, uint256 amount) internal returns (uint256) {
    ...
    if(amount > 0 ) _sakeStakeToken.convertTo(amount, to);
    return amount;
  }
   ...
   */
  function _checkOnNFTHarvest(address to, uint256 tokenId, uint256 sakeTokenAmount, uint256 sakeStakeTokenAmount) internal {
    address nftOwner = ERC721.ownerOf(tokenId);
    if (nftOwner.isContract()) {
      bytes memory returndata = nftOwner.functionCall(abi.encodeWithSelector(
          INFTHandler(nftOwner).onNFTHarvest.selector, msg.sender, to, tokenId, sakeTokenAmount, sakeStakeTokenAmount), "non implemented");
  ...
  /**
  * @dev Forbid transfer when ksNFT's owner is a contract and an operator is trying to transfer it
