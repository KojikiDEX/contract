...
/*
 * This contract is a xSake Usage (plugin) that can boost ksNFTs' yield (staking positions on NFTPools) when it
 * receives allocations from the xSakeToken contract
 */
contract YieldBooster is Ownable, ReentrancyGuard, IxSakeTokenUsage, IYieldBooster {
  ...
  // User's position xSake total allocation
  ...
  mapping(address => uint256) private _usersTotalAllocation; // User's xSake total allocation
  ...
  constructor(IxSakeToken xSakeToken_) {
    xSakeToken = xSakeToken_;
  }
  ...
  /**
   * @dev Returns total xSake allocated to this contract by "poolAddress"
   */
  ...
  /**
   * @dev Returns allocated xSake to "tokenId" ksNFT from "poolAddress" NFTPool by "userAddress"
   */
  ...
  /**
   * Allocates "userAddress" user's "amount" of xSake to this usage contract
  ...
  function allocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant xSakeTokenOnly {
    ...
    // allocated xSake is added (as boost points) to ksNFT
    INFTPool(poolAddress).boost(tokenId, amount);
  }

  /**
   * Deallocates "userAddress" user's "amount" of xSake from this usage contract
   * "data" should contain tokenId and poolAddress
   *
   * Can only be called by xSakeToken contract, which is trusted to verify amounts
   */
  function deallocate(address userAddress, uint256 amount, bytes calldata data) external override nonReentrant xSakeTokenOnly {
    ...
    // should only be called if ksNFT has not been burned, to avoid having stuck xSake on it
    if(INFTPool(poolAddress).exists(tokenId)) {
      // allocated xSake is removed (as boost points) from the ksNFT
   ...
   * The pool should remove the allocated xSake (boost points) from its own ksNFT when calling this function
   */
  function deallocateAllFromPool(address userAddress, uint256 tokenId) external override nonReentrant {
    ...
    // update user's xSake allocations balance
    xSakeToken.deallocateFromUsage(userAddress, amount);
  }

  /**
   * Deallocates msg.sender's "amount" of xSake from xSakeToken, without adjusting this contract allocations balances
   *
  ...
  function forceDeallocate() external nonReentrant {
    ...
    uint256 amount = xSakeToken.usageAllocations(msg.sender, address(this));

    // update user's xSake allocations balance
    xSakeToken.deallocateFromUsage(msg.sender, amount);
  }
  ...
  /**
   * @dev Returns multiplier that should be applied to a ksNFT based on its boost points (allocated xSake)
   *
   ...
   * 1% of the pool's allocated xSake
   *
  ...
  /**
   * @dev Allocates "userAddress" user's "amount" of xSake to "tokenId" ksNFT of "poolAddress" pool
   */
  ...
  /**
   * @dev Deallocates "userAddress" user's "amount" of xSake allocated to "tokenId" ksNFT of "poolAddress" pool
   */
  function _deallocate(address userAddress, address poolAddress, uint256 tokenId, uint256 amount) internal {
    ...
    require(userPositionAllocation >= amount, "deallocate: not enough allocated xSake");
