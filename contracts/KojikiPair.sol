...
import './interfaces/IKojikiPair.sol';
import './UniswapV2ERC20.sol';
import './libraries/Math.sol';
import './interfaces/IERC20.sol';
import './interfaces/IKojikiFactory.sol';
import './interfaces/IUniswapV2Callee.sol';

// contract KojikiPair is IKojikiPair, UniswapV2ERC20 {
contract KojikiPair is UniswapV2ERC20 {
  ...
  modifier lock() {
    require(unlocked == 1, 'KojikiPair: LOCKED');
    unlocked = 0;
    _;
    unlocked = 1;
  }
  ...
  function _safeTransfer(address token, address to, uint value) private {
    ...
    require(success && (data.length == 0 || abi.decode(data, (bool))), 'KojikiPair: TRANSFER_FAILED');
  }
  ...
  function initialize(address _token0, address _token1) external {
    require(msg.sender == factory && !initialized, 'KojikiPair: FORBIDDEN');
    // sufficient check
    token0 = _token0;
    token1 = _token1;
  ...
  */
  function setFeePercent(uint16 newToken0FeePercent, uint16 newToken1FeePercent) external lock {
    require(msg.sender == IKojikiFactory(factory).feePercentOwner(), "KojikiPair: only factory's feeAmountOwner");
    require(newToken0FeePercent <= MAX_FEE_PERCENT && newToken1FeePercent <= MAX_FEE_PERCENT, "KojikiPair: feePercent mustn't exceed the maximum");
    require(newToken0FeePercent > 0 && newToken1FeePercent > 0, "KojikiPair: feePercent mustn't exceed the minimum");
    token0FeePercent = newToken0FeePercent;
    token1FeePercent = newToken1FeePercent;
    _safeTransfer(_token0, msg.sender, IERC20(_token0).balanceOf(address(this)));
    _safeTransfer(_token1, msg.sender, IERC20(_token1).balanceOf(address(this)));
  ...
  function setStableSwap(bool stable, uint112 expectedReserve0, uint112 expectedReserve1) external lock {
    require(msg.sender == IKojikiFactory(factory).setStableOwner(), "KojikiPair: only factory's setStableOwner");
    require(!pairTypeImmutable, "KojikiPair: immutable");

    require(stable != stableSwap, "KojikiPair: no update");
    require(expectedReserve0 == reserve0 && expectedReserve1 == reserve1, "KojikiPair: failed");
  ...
  function setPairTypeImmutable() external lock {
    require(msg.sender == IKojikiFactory(factory).owner(), "KojikiPair: only factory's owner");
    require(!pairTypeImmutable, "KojikiPair: already immutable");
  ...
  function _update(uint balance0, uint balance1) private {
    // require(balance0 <= uint112(- 1) && balance1 <= uint112(- 1), 'KojikiPair: OVERFLOW');
    require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, 'KojikiPair: OVERFLOW');
  ...
  function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
    ...
    (uint ownerFeeShare, address feeTo) = IKojikiFactory(factory).feeInfo();
  ...
  function mint(address to) external lock returns (uint liquidity) {
    ...
    require(liquidity > 0, 'KojikiPair: INSUFFICIENT_LIQUIDITY_MINTED');
    _mint(to, liquidity);
  ...
  function burn(address to) external lock returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'KojikiPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function _swap(TokensData memory tokensData, address to, bytes memory data, address referrer) internal lock {
    require(tokensData.amount0Out > 0 || tokensData.amount1Out > 0, 'KojikiPair: INSUFFICIENT_OUTPUT_AMOUNT');

    (uint112 _reserve0, uint112 _reserve1, uint16 _token0FeePercent, uint16 _token1FeePercent) = getReserves();
    require(tokensData.amount0Out < _reserve0 && tokensData.amount1Out < _reserve1, 'KojikiPair: INSUFFICIENT_LIQUIDITY');


    {
      require(to != tokensData.token0 && to != tokensData.token1, 'KojikiPair: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'KojikiPair: INSUFFICIENT_INPUT_AMOUNT');
    ...
    {// scope for referer/stable fees management
      ...
      uint referrerInputFeeShare = referrer != address(0) ? IKojikiFactory(factory).referrersFeeShare(referrer) : 0;
    ...
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      ...
      require(_k(balance0Adjusted, balance1Adjusted) >= _k(uint(_reserve0), uint(_reserve1)), 'KojikiPair: K');
    }
  ...
  function sync() external lock {
    ...
    require(token0Balance != 0 && token1Balance != 0, "KojikiPair: liquidity ratio not initialized");
  ...
  */
  function drainWrongToken(address token, address to) external lock {
    require(msg.sender == IKojikiFactory(factory).owner(), "KojikiPair: only factory's owner");
    require(token != token0 && token != token1, "KojikiPair: invalid token");
