...
contract RaitoPoolFactory is Ownable, IRaitoPoolFactory {
  using EnumerableSet for EnumerableSet.AddressSet;

  ISakeToken public sakeToken; // SakeToken contract's address
  ISakeStakeToken public sakeStakeToken; // sakeStakeToken contract's address
  ...
  constructor(ISakeToken _sakeToken, ISakeStakeToken sakeStakeToken_, address emergencyRecoveryAddress_, address feeAddress_){
  ...
  function createNitroPool(
    address nftPoolAddress, IERC20 rewardsToken1, IERC20 rewardsToken2, RaitoPool.Settings calldata settings
  ) external virtual returns (address nitroPool) {

    // Initialize new nitro pool
    nitroPool = address(
      new RaitoPool(
        sakeToken, sakeStakeToken, msg.sender, INFTPool(nftPoolAddress),
   ...
   *
   * Must only be called by the RaitoPool contract
   */
