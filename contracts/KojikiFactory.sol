...
import './KojikiPair.sol';

// contract KojikiFactory is IKojikiFactory {
contract KojikiFactory {

    address public owner;
    address public feePercentOwner;
    address public setStableOwner;
    address public feeTo;
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(KojikiPair).creationCode));
     ...
     */
    modifier onlyOwner() {
        require(owner == msg.sender, "KojikiFactory: caller is not the owner");
        _;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'KojikiFactory: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'KojikiFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'KojikiFactory: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(KojikiPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(pair != address(0), "KojikiFactory: FAILED");
        KojikiPair(pair).initialize(token0, token1);
    ...
    function setOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "KojikiFactory: zero address");
        emit OwnershipTransferred(owner, _owner);
        owner = _owner;
    }

    function setSetStableOwner(address _setStableOwner) external {
        require(msg.sender == setStableOwner, "KojikiFactory: not setStableOwner");
        require(_setStableOwner != address(0), "KojikiFactory: zero address");
    ...
    function setOwnerFeeShare(uint newOwnerFeeShare) external onlyOwner {
        require(newOwnerFeeShare > 0, "KojikiFactory: ownerFeeShare mustn't exceed minimum");
        require(newOwnerFeeShare <= OWNER_FEE_SHARE_MAX, "KojikiFactory: ownerFeeShare mustn't exceed maximum");
    ...
    function setReferrerFeeShare(address referrer, uint referrerFeeShare) external onlyOwner {
        require(referrer != address(0), "KojikiFactory: zero address");
        require(referrerFeeShare <= REFERER_FEE_SHARE_MAX, "KojikiFactory: referrerFeeShare mustn't exceed maximum");
